...
Value evaluate_call(const ast::Call& call, const Location& loc, LowererContext& context) {
    ...
    if (events.empty()) {
        if (inner_cursor > 1e-9) {
            SeqVal rest_only;
            rest_only.items.push_back(std::make_shared<Value>(Value{RestVal{inner_cursor}}));
    ...
    while (event_i < events.size()) {
        ...
        if (beat - cursor > 1e-9) {
            sequence_value.items.push_back(std::make_shared<Value>(Value{RestVal{beat - cursor}}));
        }
        ...
        if (event_j - event_i == 1) {
            ...
            sequence_value.items.push_back(
                std::make_shared<Value>(Value{NoteVal{event.midi_note, event.duration_beats, event.velocity}}));
        } else {
            ChordVal chord;
            ...
            for (std::size_t k = event_i; k < event_j; ++k) {
                ...
                chord.notes.push_back(NoteVal{event.midi_note, event.duration_beats, event.velocity});
    ...
    if (inner_cursor - cursor > 1e-9) {
        sequence_value.items.push_back(std::make_shared<Value>(Value{RestVal{inner_cursor - cursor}}));
    }
