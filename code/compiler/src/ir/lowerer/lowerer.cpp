...
Program Lowerer::lower(const ast::Program& program) {
    Program out;
...
void Lowerer::lower_header(const ast::Header& header, Program& out) {
...
Track Lowerer::lower_track(const ast::TrackDefinition& track, LowererContext& ctx) {
    Track out;
    out.name = track.name;
    if (track.instrument) {
