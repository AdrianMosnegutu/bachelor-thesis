...
std::vector<NoteEvent> Lowerer::lower_play(const ast::PlayStatement& stmt, LowererContext& ctx, double& cursor) {
    ...
    if (const auto* expr_src = std::get_if<ast::ExpressionPtr>(&target.source)) {
        ...
        val = Value{NoteValue{static_cast<int>(drum), dur, 100}};
    }
        ...
        if (const auto* sv = std::get_if<SequenceValue>(&val.kind)) {
            for (const auto& item_ptr : sv->items) {
                total += std::visit(utils::overloaded{
                                        [](const NoteValue& n) -> double { return n.duration_beats; },
                                        [](const RestValue& r) -> double { return r.duration_beats; },
                                        [](const ChordValue& c) -> double { return c.duration_beats; },
